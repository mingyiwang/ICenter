    ...
    public static class TimeZones
    {
        ...
        internal static DaylightTime GetDaylightTime(TimeZoneInfo timeZone, int year)
        {
            ...
            var startDate = start.IsFixedDateRule 
                          ? Dates.Of(year, Month.Of((int) start.Month), start.Day).Add(TimeSpan.FromTicks(start.TimeOfDay.Ticks)) 
            ...
            if (end.IsFixedDateRule)
            {
                ...
                {
                    endDate = Dates.Of(year, Month.Of((int) end.Month), end.Day).Add(TimeSpan.FromTicks(end.TimeOfDay.Ticks));
                }
            }
            else
            {
