...
namespace Core.Primitive
{
    ...
    public sealed class Numbers
    {
        ...
        public static int GetInt(float value, RoundKind round = RoundKind.HalfUp)
        {
            ...
            if (Equals((float) diff, 0f) || intValue == int.MinValue || intValue == int.MaxValue)
            {
                return intValue;
            }
            ...
            switch (round)
            {
                ...
                {
                    return diff < FloatHalfOfOne || Equals((float) diff, FloatHalfOfOne)
                         ? intValue
                         : value.IsNegative() ? intValue - 1 : intValue + 1;
                }
                case RoundKind.HalfEven:
                {
                    if (Equals((float) diff, FloatHalfOfOne))
                    {
                        return (intValue & 1) == 0 
                              ? intValue 
                              : value.IsNegative() ? intValue - 1 : intValue + 1;
